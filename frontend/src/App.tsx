  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        // Task not found or server error — stop polling
        if (pollingRef.current) {
          clearInterval(pollingRef.current);
          pollingRef.current = null;
        }
    ...
    <div className="min-h-screen font-sans">
      ...
      <nav className="border-b border-[var(--border)] py-4 px-6 sticky top-0 bg-[var(--bg)]/80 backdrop-blur-md z-50">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <a href="https://github.com" target="_blank" rel="noreferrer" className="text-[var(--text)] hover:text-[var(--accent)] transition-colors">
              <Github size={24} />
            </a>
          </div>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        ...
        <section className="space-y-8">
              ...
              <div className="flex flex-col md:flex-row">
                <div className="md:w-2/5 relative aspect-video md:aspect-auto">
                  ...
                  {videoInfo.is_playlist && (
                    <div className="absolute top-4 left-4 bg-[var(--accent)] text-white text-xs font-bold px-3 py-1 rounded-full">
                ...
                <div className="md:w-3/5 p-8 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-[var(--text)] flex items-center gap-1 mb-6">
                  ...
                  >
                    ...
                    {downloading ? 'Preparing...' : 'Start Download'}
                  </button>
                </div>
              </div>
            </div>
          ...
          {progress && (
            <div className="bg-[var(--bg)] border border-[var(--border)] rounded-3xl p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-8">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h3 className="text-xl font-bold mb-1">
                    {progress.completed ? 'Download Complete!' : progress.error ? 'Download Failed' : progress.status === 'processing' ? 'Processing...' : 'Downloading...'}
                  </h3>
                  <p className="text-[var(--text)] truncate max-w-md">
                    {videoInfo?.title || 'Preparing file...'}
                  </p>
                </div>
                <div className="text-right">
                  {progress.completed ? (
                    ...
                    <div className="text-sm font-mono bg-[var(--accent-bg)] text-[var(--accent)] px-3 py-1 rounded-full border border-[var(--accent-border)]">
              ...
              {!progress.completed && !progress.error && (
                <div className="space-y-4">
                  <div className="w-full bg-[var(--code-bg)] h-3 rounded-full overflow-hidden">
                    <div 
                      className="bg-[var(--accent)] h-full transition-all duration-300 ease-out rounded-full"
                      style={{ width: `${progress.progress}%` }}
                    ></div>
                  </div>
                  <div className="flex justify-between text-sm font-medium">
                    <span className="text-[var(--accent)]">{progress.progress.toFixed(1)}% finished</span>
                    <span className="text-[var(--text)]">
                      {progress.status === 'processing' ? 'Processing audio/video...' : `ETA: ${progress.eta || 'Calculating...'}`}
                    </span>
                  </div>
                </div>
              )}
              ...
              {progress.completed && (
                <div className="flex flex-col sm:flex-row gap-4">
                  <button 
                    onClick={fetchFile}
                    className="flex-1 bg-[var(--accent)] text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:brightness-110 transition-all shadow-lg shadow-[var(--accent)]/30 cursor-pointer"
                  >
                    <Download size={24} />
                    Save to Device
                  </button>
                  <button 
                    onClick={resetAll}
                    className="bg-[var(--code-bg)] text-[var(--text-h)] px-8 py-4 rounded-xl font-bold hover:bg-[var(--border)] transition-all cursor-pointer"
                  >
                    Download Another
                  </button>
                </div>
              ...
              {progress.error && (
                <div className="space-y-4">
                  ...
                  <button 
                    ...
                    className="w-full bg-red-500 text-white py-4 rounded-xl font-bold cursor-pointer hover:bg-red-600 transition-colors"
                  >
                    Try Again
                  </button>
                </div>
